...
bool GroupePayement::clore(){
    QSqlDatabase db = DatabaseManager::getDatabase();
    QSqlQuery query;

    query.prepare("DELETE FROM " + this->getType() + " WHERE id = :id");
    query.bindValue(":id", this->id);
    if(!query.exec()) qWarning() << "Error: " << db.lastError();

    query.prepare("DELETE FROM Participants" + this->getType() + " WHERE id = :id");
    query.bindValue(":id", this->id);
    if(!query.exec()) qWarning() << "Error: " << db.lastError();
...
Utilisateur* GroupePayement::getParticipant(const QString& email){
    for (int i = 0; i < participants.size(); i++) {
