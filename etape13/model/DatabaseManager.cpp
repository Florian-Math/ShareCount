...
DatabaseManager::DatabaseManager(){
    ...
    QSqlQuery query6("CREATE TABLE if not exists Depense (id INTEGER PRIMARY KEY AUTOINCREMENT, titre TEXT NOT NULL, date TEXT NOT NULL, montant REAL NOT NULL, idCompte INTEGER NOT NULL, emailPayeur TEXT NOT NULL, emailParticipants TEXT)");
    if(!query6.isActive()) qWarning() << "Error: " << database.lastError();

    QSqlQuery query7("CREATE TABLE if not exists Transfert (id INTEGER PRIMARY KEY AUTOINCREMENT, titre TEXT NOT NULL, date TEXT NOT NULL, montant REAL NOT NULL, idCagnotte INTEGER NOT NULL, emailCreateur TEXT NOT NULL, emailValideurs TEXT)");
    if(!query7.isActive()) qWarning() << "Error: " << database.lastError();
}
