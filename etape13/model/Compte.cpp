...
bool Compte::save(){
    ...
    if(this->id != -1){
        ...
        // ajout des participants au compte
...
bool Compte::clore(){
    for (int i = 0; i < participants.size(); i++) {
        if (balances[participants[i].getEmail()] >= 0.01f) return false;
    }

    GroupePayement::clore();

    QSqlDatabase db = DatabaseManager::getDatabase();
    QSqlQuery query;

    query.prepare("DELETE FROM Depense WHERE idCompte = :idCompte");
    query.bindValue(":idCompte", this->id);
    if(!query.exec()) qWarning() << "Error: " << db.lastError();
...
void Compte::updateBalances(){
    ...
    for(int i = 0; i < depenses.size(); i++){
        ...
        for(int j = 0; j < participants.size(); j++){
            ...
            // si l'utilisateur est concerné par la depense
...
void Compte::updateEquilibrage(){
    ...
    // met les balances positive dans pos & les dépenses négative dans neg
    ...
    for(int i = 0; i < neg.size(); i++){
        ...
        // tant qu'il reste de l'argent à payer
        int j = 0;
        while (negVal < 0) {
            //if(j >= pos.size()) break;
