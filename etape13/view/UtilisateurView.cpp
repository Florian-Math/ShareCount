...
void UtilisateurView::updateSupprGroupes(GroupePayement* groupe){
    for (int i = 0; i < groupes.size(); i++) {
        GroupeView* g = groupes.at(i);
        if(g->getGroupe() == groupe){
            groupes.removeAt(i);
            g->deleteLater();
            emit groupesChanged(groupes);
            return;
...
void UtilisateurView::updateAjoutGroupe(GroupePayement* groupe) {
    if(groupe->getType() == "Compte"){
        CompteView* c = new CompteView();
        c->setGroupe(*groupe);
        groupes.insert(utilisateur->getCompteSize()-1, c);
    }
    else if(groupe->getType() == "Cagnotte"){
        CagnotteView* c = new CagnotteView();
        c->setGroupe(*groupe);
        groupes.insert(groupes.size()-1, c);
    }
