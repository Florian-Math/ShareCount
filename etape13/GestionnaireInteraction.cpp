...
void GestionnaireInteraction::deconnexion() {
    utilisateurCourant.dispose();
}
...
bool GestionnaireInteraction::cloreGroupe(){
    if(!groupeCourant->clore()) return false;

    utilisateurCourant.supprimerGroupe(groupeCourant);
    utilisateurCourantView.updateSupprGroupes(groupeCourant);

    if(groupeCourant != nullptr){
        delete groupeCourant;
        groupeCourant = nullptr;
    }
...
bool GestionnaireInteraction::ajouterDepense(const QString& titre, const QString& date, const QString& montant){
    ...
    if(montant.isEmpty()) return false;
...
bool GestionnaireInteraction::validerTransfert(const int& numTransfert){
    if(groupeCourant->getType() != "Cagnotte") return false;
    Cagnotte* c = (Cagnotte*)groupeCourant;
    CagnotteView* cv = (CagnotteView*)groupeCourantView;

    TransfertView* transfertView = cv->getTransfertsEnAttente().at(numTransfert);
    if(transfertView->getTransfert()->getCreateur().getEmail() == utilisateurCourant.getEmail()) return false; // interdiction de valider son transfert

    transfertView->getTransfert()->valider(utilisateurCourant.getEmail());

    c->updateSomme();
    cv->updateValidationTransfert();
